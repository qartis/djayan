...
unsigned int Fl_GIF::framecount()
{
	return this->FrameCount;
}

unsigned int Fl_GIF::framedelay()
{
...
void Fl_GIF::framedelay(unsigned int newdelay)
{
	unsigned int mynewdelay=(unsigned int)(newdelay/10);
	(*FrameDelay)[currframe]=mynewdelay;
}

int Fl_GIF::read_image_data(int Left,int top,int frameWidth,int frameHeight,int BitsPerPixel)
{
...
void Fl_GIF::convert_to_xpm(int Left,int Top,int frameWidth,int frameHeight,int BitsPerPixel)
{
...
void Fl_GIF::animating(int yesno)
{
...
int Fl_GIF::animating()
{
	return run_animation;
}

void Fl_GIF::select_frame(unsigned int framenum)
{
...
void Fl_GIF::animate(void* instance)
{
	Fl_GIF* mythis=(Fl_GIF*)instance;
...
unsigned int Fl_GIF::repeatcount()
{
	return this->repcount;
}

void Fl_GIF::repeatcount(unsigned int newcount)
{
	this->repcount=newcount;
}

Fl_Widget* Fl_GIF::parent()
{
	return this->_parent;
}

void Fl_GIF::parent(Fl_Widget* p_parent)
{
	this->_parent=p_parent;
}

Fl_GIF::Fl_GIF(const char *infname) : Fl_Pixmap((char *const*)0) {
  //LOGPRINT("Fl_GIF::Fl_GIF(\"%s\")\n",infname);
  ...
  if ((GifFile = fopen(infname, "rb")) == NULL) {
    Fl::error("Fl_GIF: Unable to open %s!", infname);
    return;
  }
  ...
  if (b[0]!='G' || b[1]!='I' || b[2] != 'F') {
    fclose(GifFile);
    Fl::error("Fl_GIF: %s is not a GIF file.\n", infname);
    return;
  }
  ...
  for (;;) {
    ...
    if (i < 0) {
      fclose(GifFile);
      Fl::error("Fl_GIF: %s - unexpected EOF",infname); 
      return;
    }
...
//        }
	...
	for (i=0; i < n; i++) {	
	  ...
	  if (readok!=0) {
	      Fl::error("Fl_GIF: %s - LZW Barf!", infname); 
	  } else {
